...
@section('content')
                ...
                <div class="card card-outline card-info">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-12">
                                <div class="form-group row">
                                    <label class=" ml-3 mr-3 col-form-label  text-primary"><i
                                        ...
                                        data-target="#modal-pengiriman">Pilih
                                        Pengiriman</button>
                                </div>
                                <table class="table table-borderless" id="pengiriman-pilih">
                        ...
                        <div class="col-12">
                            ...
                            <div class="btn-group mr-2" role="group" aria-label="Second group">
                                <button type="button" id="ajukan" class="btn btn-sm btn-block btn-primary">
    ...
    <!-- Modal kurir -->
    <div class="modal fade" id="modal-pengiriman" tabindex="-1" role="dialog" aria-labelledby="ajaxModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <table class="table table-hover va-middle table-borderless table-striped" id="tabel-pengiriman">
                    </table>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
                    <button type="button" class="btn btn-primary" id="btn-konfirmasi-pengiriman">Konfirmasi</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    <script>
        $(function() {
            ...
            // kumpulan perintah untuk Kurir
            var tabelPengiriman = $('#tabel-pengiriman').DataTable({
                autoWidth: false,
                lengthChange: false,
                info: false,
                searching: false,
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        'data': function(data, type, full, meta) {
                            return `
                            <div class="row">
                                <div class="col-12 font-weight-bold" style="font-size:20px">${data.courier_name} <span class="pl-2">Rp ${numeral(data.price).format()}</span></div>
                                <div class="col-12 ">${data.description}</div>
                                <div class="col-12 font-weight-light">Durasi Pengiriman :${data.duration}</div>
                                <div class="col-12 font-weight-light">Jenis Pengiriman : ${data.shipping_type} </div>
                            </div>
                            `
                        },
                    },
            ...
            $('#modal-pengiriman').on('show.bs.modal', function(event) {
                $.ajax({
                    type: "post",
                    url: '{{ url('penjualan/getrate') }}',
                    data: {
                        pelanggan: kirimPelanggan[0] ?? null,
                        alamat: alamatsaya[0] ?? null,
                        item: keranjang[0] ?? null
                    },
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    dataType: "json",
                    success: function(response) {
                        tabelPengiriman.clear();

                        daftarkurir = response.data;
                        tabelPengiriman.rows.add(daftarkurir);
                        tabelPengiriman.draw();
                    ...
                    beforeSend: function() {
                        $('#tabel-pengiriman').hide()
                        $('#loading').show();
                    },
                    complete: function() {
                        // Hide the loading indicator after the request is complete (success or error)
                        $('#loading').hide();
                        $('#tabel-pengiriman').show()
                    }
                });

            });

            $('#tabel-pengiriman').on('click', 'tr', function() {
                var row = tabelPelanggan.row(this);
            ...
            var tabel_kurir_pilih = $('#pengiriman-pilih').DataTable({
                ...
                columns: [{
                        'data': function(data, type, full, meta) {
                            return `
                            <label class="text-weight-bold">${data.courier_name}</label>
                            `;
                        }
                    },
                    {
